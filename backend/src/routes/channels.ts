...
import { botToken, allowedTopics, updateChannelStats, requireIntParam } from "../helpers";
import { asyncHandler } from "../asyncHandler";

const router = Router();

router.post("/channels/register", asyncHandler(async (req, res) => {
  try {
    const telegramId = Number(req.body?.telegramId);
    const channel = String(req.body?.channel ?? "");

    if (!telegramId || !channel) {
      return res.status(400).json({ error: "telegramId and channel are required" });
    }
    ...
    const botMember = await getChatMember(botToken, String(chat.id), botUser.id);
    ...
    await pool.query(
      ...
      `,
      [chat.id, chat.title ?? "Untitled", chat.username ?? null, userRows[0].id],
    );
...
router.get("/channels", asyncHandler(async (req, res) => {
  ...
  if (minSubscribers !== null) addCondition("subscribers >= ?", minSubscribers);
  if (maxSubscribers !== null) addCondition("subscribers <= ?", maxSubscribers);
...
router.get("/channels/my/:telegramId", asyncHandler(async (req, res) => {
...
router.patch("/channels/:telegramId/card", asyncHandler(async (req, res) => {
  const telegramId = requireIntParam(req.params.telegramId, "telegramId", { allowNegative: true });
  ...
  const payoutAddress =
    req.body?.payoutAddress !== undefined ? String(req.body.payoutAddress) : null;
